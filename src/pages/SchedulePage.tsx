...
export function SchedulePage() {
  ...
  const { campuses, departments, fetchCampuses, fetchDepartments, selectedCampusId, setSelectedCampus, fetchBookingSetting, bookingSetting, updateBookingSetting, fetchEmailTemplates, emailTemplates, upsertEmailTemplate, fetchScheduleConfig, dayOverrides, fetchDayOverrides } = useScheduleStore();
  ...
  // ── Settings ──
  // editingMaxBookings removed - field is always editable
  ...
  // ── Day Settings state ──
  const [dayOverride, setDayOverride] = useState<import('~/types').DayOverride | null>(null);
  const [dayMaxBookings, setDayMaxBookings] = useState(50);
  const [dayIsClosed, setDayIsClosed] = useState(false);
  const [dayNotes, setDayNotes] = useState('');
  // dayOverrides moved to store
  ...
  const handleDateClick = (date: Date) => {
    ...
    setWalkInName(''); setWalkInContact(''); setWalkInEmail(''); setWalkInDepartment(''); setWalkInNotes('');
  ...
  const handleSaveDaySettings = async () => {
    ...
    try {
      ...
      const start = startOfMonth(subMonths(currentMonth, 1));
      const end = endOfMonth(addMonths(currentMonth, 1));
      await fetchDayOverrides(selectedCampusId, formatLocalDate(start), formatLocalDate(end));
  ...
  const handleRemoveDayOverride = async () => {
    if (!selectedDate || !dayOverride?.id || !selectedCampusId) return;
            ...
            <div className="ml-2">
              <div className="flex items-center gap-2 px-4 py-2 rounded-lg border bg-white shadow-sm transition-all duration-200 border-gray-300 focus-within:border-maroon-800 focus-within:ring-2 focus-within:ring-maroon-200 focus-within:ring-offset-1">
                ...
                <input
                  ...
                  className="w-[4ch] bg-transparent p-0 border-none outline-none text-sm font-bold text-gray-900 placeholder-gray-300 focus:ring-0 appearance-none text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                />
              </div>
            </div>
          )}
