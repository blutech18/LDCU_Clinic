...
export function PrivacyPolicyPage({ embedded = false }: PrivacyPolicyPageProps) {
  const navigate = useNavigate();
  const { profile, isAuthenticated } = useAuthStore();

  const handleBack = () => {
    // Go back to where user came from, with role-aware fallback when no history.
  ...
  const content = (
    <main className="flex-1 w-full">
      ...
      <div className="bg-gradient-to-r from-maroon-900 via-maroon-800 to-maroon-900 w-full text-white border-b-4 border-maroon-950">
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-9">
          <div className="flex items-center gap-3 sm:gap-4">
            {/* Back button pinned to far-left with formal page margin */}
            <button
              onClick={handleBack}
            ...
            {/* Main heading block centered independently of back button */}
            <div className="flex-1 flex justify-center min-w-0">
              <div className="w-full max-w-4xl flex items-center gap-3 sm:gap-5 min-w-0">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-1 lg:gap-4 min-w-0">
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight whitespace-nowrap shrink-0 leading-none">
                    Privacy Policy
                  </h1>
                  <span className="text-maroon-400 text-3xl font-light hidden lg:block pb-1 shrink-0">|</span>
                  <p className="text-maroon-100 text-sm sm:text-lg font-medium whitespace-nowrap truncate min-w-0">
  ...
  if (embedded) {
    return <div className="min-h-full flex flex-col bg-white">{content}</div>;
  }
